...
then
  ...
  # shellcheck source=scripts/jenkins.properties
...
while getopts ":p:c:r:e:d:n:h;" opt; do
  case ${opt} in
    p) JENKINS_PROJECT=${OPTARG};;
    c) JENKINS_CLUSTER_NAME=${OPTARG};;
    e) ENV=${OPTARG};;
    d) export REPO_DIR=${OPTARG};;
    n) export REPO_NAME=${OPTARG};;
    h) usage;;
    \?)
...
if [ -z "${ZONE}" ]; then
    ...
    echo "replace 'ZONE' with a zone name like us-west1-a." 1>&2
    exit 1;
fi
